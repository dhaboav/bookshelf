...
export function BookCard({ book, actionsSlot }: Props) {
  return (
      ...
      <div className="flex w-full flex-col justify-center overflow-hidden px-4 py-2">
        <div className="flex flex-row items-center justify-between">
          <p className="text-primary line-clamp-1 text-xs font-semibold">
            {book.genre?.name?.toUpperCase() ?? 'UNKNOWN'}
          </p>
          {actionsSlot}
        </div>
        ...
        <div className="mt-1.5 grid grid-cols-3 items-center">
          <div className="flex flex-row items-center gap-x-1">
            ...
            {/* Fallback text if author is null */}
            <span className="text-[10px] font-semibold">{book.author?.name ?? 'Unknown'}</span>
          </div>
